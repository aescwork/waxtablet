...
..	currentmodule:: waxtablet
	...
	:synopsis:  a python application to create, read, update and delete log entries, and display them via html.
...
class SQLiteMinor(object):
	...
	def cursor(self):
		""" 
		Get the cursor (call it directly: obj.cursor)
		"""
		return self._cursor
	...
	def add(self, members):
		...
		try:
			...
			ret_val = self.exec_stat("INSERT INTO " + self.table + " VALUES" + placeholders, "add", (members))
	...
	def read_all(self, select_column="*"):
		...
		ret_val = self.exec_stat("SELECT " + select_column + " FROM " + self.table,  "read_all")
	...
	def read_range(self, frm, to, col,  select_column="*"):
		...
		try:	
			...
			ret_val = self.exec_stat(statement, "read_range", (frm, to)) 
			self._set_result_and_status("OK", "")

		except Exception as e:
			msg = "In sqliteminor read_range(): ", str(e)
			self._set_result_and_status("FAIL", msg)
	...
	def read_head(self, number=5,  select_column="*"):
		...
		try:
			...
			ret_val = self.exec_stat("SELECT " + select_column + " FROM " + self.table + " LIMIT ?", "read_head", (number, ))
			self._set_result_and_status("OK", "")

		except Exception as e:
			msg = "In sqliteminor read_head(): ", str(e)
			self._set_result_and_status("FAIL", msg)
	...
	def read_tail(self, number=5, col=None, select_column="*"):
		...
		try:
			...
			ret_val = self.exec_stat("SELECT " + select_column + " FROM " + self.table + " ORDER BY " + col + " DESC LIMIT ?", "read_tail", (number, ))
			self._set_result_and_status("OK", "")

		except Exception as e:
			msg = "In sqliteminor read_tail(): ", str(e)
			self._set_result_and_status("FAIL", msg)
	...
	def read_search_rows(self, target_col, search_val, search_scope="has-any", select_column="*"):
		...
		try:
			...
			ret_val = self.exec_stat("SELECT " + select_column + " FROM " + self.table + stmt_parts[0], "read_search_rows", stmt_parts[1])
			self._set_result_and_status("OK", "")

		except Exception as e:
			msg = "In sqliteminor read_search_rows(): ", str(e)
			self._set_result_and_status("FAIL", msg)
	...
	def read_by_date_part(self, part, column, search_val, select_column="*"):
		...
		try:
			...
			ret_val = self.exec_stat("SELECT " + select_column + " FROM " + self.table + " WHERE strftime('" + subs[part] + "', datetime(" + column + ")) = ?", \
					"read_by_date_part",  (search_val, ))
			self._set_result_and_status("OK", "")

		except Exception as e:
			msg = "In sqliteminor read_by_date_part(): ", str(e)
			self._set_result_and_status("FAIL", msg)
	...
	def delete(self, ref_col, ref_col_val, search_scope="has-any"):
		...
		try:
			...
			ret_val = self.exec_stat("DELETE FROM " + self.table + stmt_parts[0], "delete", stmt_parts[1])
			self._set_result_and_status("OK", "")

		except Exception as e:
			msg = "In sqliteminor delete(): ", str(e)
			self._set_result_and_status("FAIL", msg)
	...
	#	stmt_parts = self._make_operator(target_col, search_val, search_scope)
		...
		for c in col:
			if type(val[col_index]) is list:
				search_list = val[col_index]
				if len(val) > col_index:						
				...
				for v in search_list:
					ret_tuple.append(v)
					statement = statement + " " + c + " LIKE ? " + operator 

		if operator == "AND":
			statement = statement[:-4]		# get rid of the last operator from the statement
		elif operator == "OR":
			statement = statement[:-3]		# get rid of the last operator from the statement
